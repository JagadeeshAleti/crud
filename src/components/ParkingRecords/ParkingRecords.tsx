...
import { Button, DataTable, DateTimePicker, FormField, IconButton, IDataFunction, IDataTableColumn, Input, SearchBox, useToast } from 'uxp/components';
...
const ParkingRecords: React.VoidFunctionComponent<IParkingRecordsProps> = ({ uxpContext }) => {
    ...
    let getParkingRecordsDataOnFilterButtonClick = React.useCallback((max: number, last: string) => (
        new Promise<{ items: any[], pageToken: string }>((resolve) => {
            setIsLoading(true);
            uxpContext.executeAction('Parking', 'ParkingHistoryOfVehicle', { max, last, ...filterData }, { json: true })
                .then(res => {
        ...
        <div className="page-content">
            <div className="parking-records-page">
                ...
                <div className="section" id="parking-spot-section">
                    <h3 className="title">Parking Records</h3>
                    <div className="form-field-group">
                        <ValidatableForm
                            validateSchema={{
                                ...
                                'dateFrom': { fieldName: 'dateFrom', label: 'Start date & time', validatorCollection: ['isRequired'] },
                                'dateTo': { fieldName: 'dateTo', label: 'End date & time', validatorCollection: ['isRequired'] },
                            }}
                            ref={formRef}
                            disabled={false}>
                            ...
                            <ValidatableFormField className="vehicle-name-field" fieldName="Vehicle.vehiclePlateIdentifier" label="Vehicle plate #" value={filterData?.['Vehicle.vehiclePlateIdentifier']}>
                                <Input
                                    ...
                                    value={filterData?.['Vehicle.vehiclePlateIdentifier'] ?? ''}
