...
const Devices: React.FunctionComponent<IDevicesProps> = (props) => {
    ...
    const mapActionData = React.useCallback((item) => {
        ...
        let ipAddress = item["Device.ipAddress"];
        if (Array.isArray(ipAddress)) ipAddress = ipAddress.join(", ")
...
const CreateDevice: React.FunctionComponent<{ uxpContext: IContextProvider }> = (props) => {
    ...
    let structure: IFormFieldDefinition[] = [
        ...
        },
        {
            ...
            type: "string",
            validate: {
                regExp: /^((((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\,?)*)-?)*$/g
            }
        },
        {
            label: "Value",
